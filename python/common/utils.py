...
def setupfigure(
    ...
    plotter_settings: PlotterSettings = PlotterSettings(),
):
    """
    ...
    # Projections
    if plotter_settings.projections:
        ax.plot(
            np.ones(g.shape[0]) * ax.get_xlim()[0],
            g[:, 13],
            g[:, 14],
            linewidth=2,
            color="r",
        ...
        ax.plot(
            g[:, 12],
            np.ones(g.shape[0]) * ax.get_ylim()[0],
            g[:, 14],
            linewidth=2,
            color="g",
        )  # project in x-z axis
        ax.plot(
            g[:, 12], g[:, 13], np.zeros(g.shape[0]), linewidth=2, color="b"
        )  # project in x-y axis

    # Base Plate
    if plotter_settings.baseplate:
        color = [0.9, 0.9, 0.9]
        squaresize = 0.02
        thickness = 0.001
        ...
        ax.add_collection3d(
            Poly3DCollection(verts, color=color, rasterized=True, zorder=-1), zdir="z"
        )
    ...
    # Coordinate Frames
    if plotter_settings.tipframe and not plotter_settings.segframe:
        ax.quiver(
            g[-1, 12],
            g[-1, 13],
            g[-1, 14],
...
def robotindependentmapping(
    kappa: np.ndarray[float],
    phi: np.ndarray[float],
    ell: np.ndarray[float],
    pts_per_seg: np.ndarray[int],
) -> np.ndarray[float]:
    """
