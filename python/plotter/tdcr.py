...
from common.types import CRDiscreteCurve, TDCRPlotterSettings
...
def draw_tdcr(
    ...
    g = curve.g
    seg_end = curve.seg_end
    r_disk = plotter_settings.r_disk
    r_height = plotter_settings.r_height
    ...
    for i in range(numseg):
        ax.plot(
            g[start : seg_end[i], 12],
            g[start : seg_end[i], 13],
            g[start : seg_end[i], 14],
            linewidth=5,
            color=col[i] * np.ones(3),
        )
        start = seg_end[i]
    ...
    r2 = np.array(
        [np.cos(30 * np.pi / 180) * r_disk, -np.sin(30 * np.pi / 180) * r_disk, 0]
    )
    r3 = np.array(
        [-np.cos(30 * np.pi / 180) * r_disk, -np.sin(30 * np.pi / 180) * r_disk, 0]
    )
    ...
    for i in range(numseg):
        ax.plot_surface(
            x * radius + tendon1[seg_end[i] - 1, 0],
            y * radius + tendon1[seg_end[i] - 1, 1],
            z * radius + tendon1[seg_end[i] - 1, 2],
            color="k",
        )
        ax.plot_surface(
            x * radius + tendon2[seg_end[i] - 1, 0],
            y * radius + tendon2[seg_end[i] - 1, 1],
            z * radius + tendon2[seg_end[i] - 1, 2],
            color="k",
        )
        ax.plot_surface(
            x * radius + tendon3[seg_end[i] - 1, 0],
            y * radius + tendon3[seg_end[i] - 1, 1],
            z * radius + tendon3[seg_end[i] - 1, 2],
            color="k",
        )
    ...
    for i in range(g.shape[0]):
        ...
        normal = RotMat[:3, 2].T
        v = nullspace(normal)
        v_theta = v[:, 0].reshape((-1, 1)) * np.cos(theta) + v[:, 1].reshape(
            (-1, 1)
        ) * np.sin(theta)
        ...
        ax.add_collection3d(
            Poly3DCollection(
                verts, color=color, edgecolor="k", rasterized=True, zorder=10
            ),
            zdir="z",
        )
